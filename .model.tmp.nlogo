...
to setup
 ;jayneel
  ...
  ;size of world set to -20- to 20 for x and y axes.

  set no-column ceiling sqrt countries
  set no-row ceiling sqrt countries
  print(no-column)
  print(no-row)
 ...
 ;jayneel
  ...
  [ ;print(i)
    ...
    [
      ...
      set j j + 1
       ;stopping condition
      if ( (i * no-row ) + j > countries [ set j no-column + 1  set i no-row + 1  ]
      ]
   set i i + 1
   set j 1
   set y y + 2 * radius
